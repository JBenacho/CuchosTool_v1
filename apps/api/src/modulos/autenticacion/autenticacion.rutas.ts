...
export async function rutasAutenticacion(aplicacion: FastifyInstance): Promise<void> {
  aplicacion.post<{ Body: CuerpoRegistro }>(
    '/autenticacion/registro',
    {
      schema: {
        ...
        body: {
          type: 'object',
          required: ['correo', 'contrasena', 'nombre'],
          properties: {
            correo: { type: 'string' },
    ...
    async function (solicitud, respuesta) {
      ...
      const [cliente] = await base
        .insert(clientes)
        .values({ correo: correo, hashContrasena: hashContrasena, nombre: nombre })
        .returning({ id: clientes.id, correo: clientes.correo, nombre: clientes.nombre });
      const token = (aplicacion as any).jwt.sign({ sub: String(cliente.id), rol: ROL_CLIENTE });
      return { data: { cliente: cliente, token: token } };
    },
  );

  aplicacion.post<{ Body: CuerpoIngreso }>(
    '/autenticacion/ingreso',
    {
      schema: {
        ...
        body: {
          ...
          properties: { correo: { type: 'string' }, contrasena: { type: 'string' } },
        },
      },
    },
    async function (solicitud, respuesta) {
      ...
      return {
        data: {
          cliente: { id: cliente.id, correo: cliente.correo, nombre: cliente.nombre },
          token: token,
        },
      };
    },
  ...
  aplicacion.post<{ Body: CuerpoIngreso }>(
    ...
    async function (solicitud, respuesta) {
      ...
      if (!usuario || usuario.estado !== ESTADO_ACTIVO)
      ...
      if (!contrasenaValida) return respuesta.code(401).send({ error: 'credenciales_invalidas' });
      const token = (aplicacion as any).jwt.sign({
