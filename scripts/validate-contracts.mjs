...
for (const f of files) {
  ...
  for (const key of ['eventId', 'type', 'version', 'occurredAt', 'data']) {
    if (!required.includes(key)) { console.error('FALTA required: ' + key + ' en ' + f); failures++; }
  }
  ...
  if (typeof typeConst !== 'string' || !typeConst.startsWith('com.cuchostool.')) {
    console.error('type.const invalido en ' + f); failures++;
  }
  const dataRequired = parsed.properties && parsed.properties.data && (parsed.properties.data.required || []);
  if (!dataRequired || dataRequired.length === 0) { console.error('data.required vacio en ' + f); failures++; }
}

if (failures > 0) { console.error(failures + ' errores de contrato'); process.exit(1); }
