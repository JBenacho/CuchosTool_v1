...
import { formatearPrecioPesos, listarProductosActivos, obtenerProductoActivo, type ProductoResumen } from './catalogo.servicio';
...
export async function rutasCatalogo(aplicacion: FastifyInstance): Promise<void> {
  aplicacion.get<{ Querystring: ConsultaProductos }>('/catalogo/productos', {
    schema: {
      tags: ['catalogo'],
      ...
      querystring: {
        type: 'object',
        properties: { q: { type: 'string' }, limit: { type: 'string' }, offset: { type: 'string' } },
      },
      response: {
        200: {
          type: 'object',
          properties: {
            ...
            meta: { type: 'object', properties: { limite: { type: 'integer' }, desplazamiento: { type: 'integer' }, cantidad: { type: 'integer' } } },
          },
        },
      },
    },
  }, async function (solicitud) {
    const resultado = await listarProductosActivos(solicitud.query.q || '', solicitud.query.limit, solicitud.query.offset);
    ...
    return { data: productosFormateados, meta: { limite: resultado.limite, desplazamiento: resultado.desplazamiento, cantidad: productosFormateados.length } };
  });

  aplicacion.get<{ Params: { id: string } }>('/catalogo/productos/:id', {
    schema: {
      tags: ['catalogo'],
